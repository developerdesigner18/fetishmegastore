...
@section('section_body')
    <div class=" bg-white rounded-lg shadow-sm p-3 text-stone-600">
        <form method="POST" enctype="multipart/form-data" action="{{route('save.short.videos')}}">
            @csrf
            ...
            <x-input id="title" class="block mt-0 w-full" type="text" value="" name="title" required/>
           ...
           <div>
            ...
            <x-label class="mt-5">{{ __("Video File") }}</x-label>
                {{--            <x-input id="video" class="block mt-0 w-full" type="file" name="video"/>--}}

            <select name="video" class="w-full selectVideo">
                <option value="">Please select</option>
                @foreach($fileNames as $videodetails)
                    <option value="{{ $videodetails['value'] }}">{{ $videodetails['label'] }}</option>
                @endforeach
            </select>
...
@push('adminExtraJS')
    ...
    <script !src="">
        ...
        // new TomSelect('.selectVideo',settings);
        // new TomSelect('.selectVideoLink',settings);
    </script>
    <script>
        // Is object mein hum saare translations save karenge. Add page par yeh khaali rahenge.
        ...
        function setupLanguageField(selectorId, wrapperId, inputId, labelId, translations) {
            ...
            function updateVisibility() {
                ...
                if (selectedLang) {
                    ...
                    // Add page par, input hamesha khaali rahega
