...
@section('section_title')
    ...
    <a href="/admin/audios" class="text-indigo-700 hover:underline font-semibold">&raquo; {{ __('Back To audios') }}</a>
@endsection
...
/* Chrome, Safari, Edge, Opera */
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 text-stone-600">
        <form method="POST" enctype="multipart/form-data" action="/admin/audios/save/{{ $audio->id }}">
            @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-input id="title" class="block mt-1 w-full" type="text"
                                 value="{{ $ogValue ? $ogValue->title->en ?? $ogValue->title->en : '' }}"
                                 name="title" required/>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="title_lang" id="title_lang_selector" class="w-full">
                            ...
                            @foreach($supportedLocales as $locale)
                                <option value="{{ $locale['lang_value'] }}" @if($audio->title_lang == $locale['lang_value']) selected @endif>
                                    {{ $locale['lang_name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        @php
                            $selectedCategories = explode(',', $audio->category_id);
                        @endphp
                    ...
                    <div>
                        @php
                            $selectedModels = explode(',', $audio->model_id);
                        @endphp
                    ...
                    <div>
                        @php
                            $selectedTags = explode(',', $audio->tags);
                        @endphp
                    ...
                    <div>
                        ...
                        <x-select name="free_for_subs" class="w-full">
                            <option value="yes" @if($audio->free_for_subs == "yes") selected @endif>{{ __("Yes") }}</option>
                            <option value="no" @if($audio->free_for_subs == "no") selected @endif>{{ __("No") }}</option>
                        </x-select>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="video" class="w-full form-control fileSelector">
                            <option value="videos/{{$audio->audio_file}}" selected>{{$audio->audio_file}}</option>
                            @foreach($fileNames as $fileName)
                                <option value="{{$fileName['value']}}" @if($audio->audio_file == $fileName['label']) selected @endif>{{$fileName['label']}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <x-input id="price" class="block mt-1 w-full" type="number" value="{{ $audio->price }}"  name="price" step="any" required/>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="description_lang" id="description_lang_selector" class="w-full">
                            ...
                            @foreach($supportedLocales as $locale)
                                <option value="{{ $locale['lang_value'] }}" @if($audio->description_lang == $locale['lang_value']) selected @endif>
                                    {{ $locale['lang_name'] }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="seo[h2]" value="{{@$audio->seoDetails->h2 ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Keyword:</label>
                            <input type="text" name="seo[keyword]" value="{{@$audio->seoDetails->keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Keyword:</label>
                            <input type="text" name="seo[meta_keyword]" value="{{@$audio->seoDetails->meta_keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Description:</label>
                            <input type="text" name="seo[desc]" value="{{@$audio->seoDetails->desc ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">H2</label>
                            <input type="text" name="seo[de][h2]" value="{{@$audio->seoDetails->de->h2 ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Keyword:</label>
                            <input type="text" name="seo[de][keyword]" value="{{@$audio->seoDetails->de->keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Keyword:</label>
                            <input type="text" name="seo[de][meta_keyword]" value="{{@$audio->seoDetails->de->meta_keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Description:</label>
                            <input type="text" name="seo[de][desc]" value="{{@$audio->seoDetails->de->desc ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                    ...
                    <div class="col-span-2 grid grid-cols-1 grid-cols-1 gap-6">
                        <div>
                            ...
                            <input id="og-title" type="text" name="seo[og_title]" value="{{@$audio->seoDetails->og_title}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="og-desc" class="block text-sm font-medium text-gray-700">OG Description:</label>
                            <input id="og-desc" type="text" name="seo[og_desc]" value="{{@$audio->seoDetails->og_desc}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="meta-robot" class="block text-sm font-medium text-gray-700">Meta Robot:</label>
                            <input id="meta-robot" type="text" name="seo[meta_robot]" value="{{@$audio->seoDetails->meta_robot}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="cust-url" class="block text-sm font-medium text-gray-700">Custom URL:</label>
                            <input id="cust-url" type="text" name="seo[cust_url]" value="{{@$audio->seoDetails->cust_url}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="og-image-url" class="block text-sm font-medium text-gray-700">OG Image URL:</label>
                            <input id="og-image-url" type="text" name="seo[og_image_url]" value="{{@$audio->seoDetails->og_image_url}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="json-id" class="block text-sm font-medium text-gray-700">JSON ID:</label>
                            <input id="json-id" type="text" name="seo[json_id]" value="{{@$audio->seoDetails->json_id}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                </div>
