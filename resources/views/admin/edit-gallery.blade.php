...
@section('section_body')
    <div class=" bg-white rounded-lg shadow-sm p-3 text-stone-600">
        <form method="POST" enctype="multipart/form-data" action="{{route('update.gallery')}}">
            @csrf
            <input type="hidden" name="id" id="galleryId" value="{{$video->id}}">
            ...
            <x-input id="title_en" class="block mt-1 w-full" type="text" value="{{ $ogValue->title->en ?? '' }}" name="title" required/>
            ...
            <div>
                ...
                <select name="title_lang" id="title_lang_selector" class="w-full">
                    ...
                    @foreach($supportedLocales as $locale)
                        <option value="{{ $locale['lang_value'] }}" @if(isset($ogValue->title_lang) && $video->title_lang == $locale['lang_value']) selected @endif>
                            {{ $locale['lang_name'] }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4" id="imageViewer">
                @if($video->galleryUrl)
                    @foreach($video->galleryUrl as $url)
                        <div class="relative">
                            <!-- Delete Icon -->
                            <a href="javascript:void(0);" class="deleteImage absolute top-1 right-1 bg-white rounded-full p-1 shadow-md" data-path="{{$url['path']}}">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="rgba(255,0,0,1)">
                                    <path d="M12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22ZM12 10.5858L9.17157 7.75736L7.75736 9.17157L10.5858 12L7.75736 14.8284L9.17157 16.2426L12 13.4142L14.8284 16.2426L16.2426 14.8284L13.4142 12L16.2426 9.17157L14.8284 7.75736L12 10.5858Z"></path>
                                </svg>
                            </a>
                            <!-- Image -->
                            <img src="{{$url['link']}}" alt="Gallery Image" class="block w-full h-48 object-cover rounded-lg shadow-md">
                        </div>
                    @endforeach
                @endif
            </div>
            ...
            <x-textarea id="description_en" class="block mt-0 w-full" name="description">
                {{ $ogValue->description->en ?? '' }}
            </x-textarea>
            ...
            <div>
                ...
                <select name="description_lang" id="description_lang_selector" class="w-full">
                    <option value="">-- No Additional Language --</option>
                    @foreach($supportedLocales as $locale)
                        <option value="{{ $locale['lang_value'] }}" @if(isset($video->description_lang) && $video->description_lang == $locale['lang_value']) selected @endif>
                            {{ $locale['lang_name'] }}
                        </option>
                    @endforeach
                </select>
...
@push('adminExtraJS')
    ...
    <script>
        // Controller se aaye raw JSON object ko JavaScript object mein badlein
        const titleTranslations = @json($ogValue->title ?? new stdClass());
        const descriptionTranslations = @json($ogValue->description ?? new stdClass());

        function setupLanguageField(selectorId, wrapperId, inputId, labelId, isTitle, translations) {
            ...
            function updateVisibility() {
                ...
                if (selectedLang) {
                    ...
                    // `translations` object se purani value nikal kar input mein daalo
                    input.val(translations[selectedLang] || '');
        ...
        $(document).ready(function() {
            // Title ke liye
            setupLanguageField('title_lang_selector', 'title_de_wrapper', 'title_de', 'title_de_label', true, titleTranslations);
            // Description ke liye
            setupLanguageField('description_lang_selector', 'description_de_wrapper', 'description_de', 'description_de_label', false, descriptionTranslations);
        });
    </script>
