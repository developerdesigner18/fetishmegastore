    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mt-5 flex md:flex-row flex-col md:space-x-5 space-y-10 md:space-y-0">
            <div class="md:w-2/3 w-full">
                <dl>
                    <dt class="font-semibold text-stone-600">{{ __('Commission(%)') }}</dt>
                    <dd>
                        <x-input type="text" name="commission_percentage" value="{{ opt('commission_percentage') }}" class="md:w-2/3 w-full" />
                    </dd>
                    <br>
                    <dt class="font-semibold text-stone-600">{{ __('Minimum Withdrawal Limit') }}</dt>
                    <dd>
                        <x-input type="text" name="minimum_withdrawal_limit" value="{{ opt('minimum_withdrawal_limit') }}" class="md:w-2/3 w-full" />
                    </dd>
                    <br>
                     <dt class="font-semibold text-stone-600">{{ __('Currency') }}</dt>
                    <dd>
                        <x-input type="text" name="currency" value="{{ opt('currency') }}" class="md:w-2/3 w-full" />
                    </dd>
                    <br>
