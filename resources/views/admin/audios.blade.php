...
<strong>{{ __('Audios Uploaded by Streamers') }}</strong>
@endsection
...
@section('section_body')
    ...
    <div class="my-5 bg-gray-200 text-gray-500 font-semibold border-2 border-gray-300 p-3 rounded">
        {{ __('Here you will find an overview of the Audios uploaded by the streamers.') }}
    </div>

    @if (count($audios))
        <div class="items-center justify-center p-5">
            <form action="{{ route('admin.audios') }}" method="GET" class="flex flex-wrap gap-3 w-full">
                <div class="flex flex-col flex-1 min-w-[180px]">
                    <label>Tag: </label>
                    <select name="tag[]" class="form-control block" id="tagSelect" multiple>
                        <option value="">{{"Select Tags For Filter Audio"}}</option>
                        @foreach($tag as $t)
                ...
                <div class="flex flex-col flex-1 min-w-[180px]">
                    ...
                    <select name="category[]" class="form-control block" id="categorySelect" multiple>
                        <option value="">{{"Select Category For Audio"}}</option>
                ...
                <div class="flex flex-col flex-1 min-w-[180px]">
                    ...
                    <select name="model[]" class="form-control block" id="modelSelect" multiple>
                        <option value="">{{"Select Model For Audio"}}</option>
                        @foreach($model as $m)
                ...
                <div class="flex flex-col flex-1 min-w-[180px]">
                    ...
                    <select name="channels[]" class="form-control block" id="channelsSelect" multiple>
                        <option value="">{{"Select Channels For Audio"}}</option>
                ...
                <div class="flex flex-col flex-1 min-w-[180px]">
                    ...
                    <x-input name="search" class="form-control block" id="search" type="text" placeholder="Search for audio"/>
                </div>
            ...
            @foreach($audios as $v)
                <div class="mr-5 mb-5 border-2 rounded-lg">
                    <div class="relative bg-gray-400 text-center">
                        <i class="fa-solid fa-music text-white text-2xl mb-1 mt-1"></i>
                        <a target="_blank" href={{route("admin.audios", [ "audio"=> $v->id, "slug" => $v->slug ])}}
                    ...
                    <div class="flex items-start p-2">
                        ...
                        <div class="text-right p-5 w-10">
                            <a href="{{ route('admin.audios.edit', ['audio' => $v->id]) }}" class="text-teal-600 hover:text-teal-800" title="Edit">
                                <i class="fa-solid fa-pencil"></i>
                            </a>
                            <a href="{{ route('admin.audios') }}?remove={{ $v->id }}" onclick="return confirm('{{ __('Are you sure you want to remove this AUDIO?') }}')" class="text-red-400 hover:text-red-600 ml-2" title="Remove">
    ...
    @else
        <div class="bg-white p-3 rounded">
            @if(request()->filled('search'))
                {{ __('No Audios matching ":searchTerm"', ['searchTerm' => request('search')]) }}
                <a href="/admin/audios" class="text-cyan-600 hover:underline">{{ __("Reset Search") }}</a>
            @else
                {{ __('No Audios uploaded by streamers.') }}
                <a href="/admin/audios" class="text-cyan-600 hover:underline">{{ __("Reset Search") }}</a>
            @endif
        </div>
    @endif
