...
/* Chrome, Safari, Edge, Opera */
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 text-stone-600">
        <form method="POST" enctype="multipart/form-data" action="/admin/videos/save/{{ $video->id }}">
            @csrf
                    ...
                    <div>
                        ...
                        <select name="title_lang" id="title_lang_selector" class="w-full">
                            ...
                            @foreach($supportedLocales as $locale)
                                <option value="{{ $locale['lang_value'] }}" @if($video->title_lang == $locale['lang_value']) selected @endif>
                                    {{ $locale['lang_name'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="model_id[]" class="w-full multiSelectorModel" multiple="">
                            @foreach($models as $modelDetails)
                                <option value="{{ $modelDetails->id }}" @if(in_array($modelDetails->id,$video->model_id)) selected @endif>{{ $modelDetails->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tags[]" class="w-full multiSelectorTag" multiple>
                            @foreach($tags as $tagDetails)
                                <option value="{{ $tagDetails->name }}" @if(in_array($tagDetails->name, $video->tags)) selected @endif>{{ $tagDetails->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="video" class="w-full fileSelector">
                            <option value="videos/{{$video->videoName}}" selected>{{$video->videoName}}</option>
                    ...
                    <div>
                        ...
                        <x-input id="price" class="block mt-1 w-full" type="number"
                                 value="{{ $video->price }}"  name="price" step="any" required/>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="description_lang" id="description_lang_selector" class="w-full">
                            ...
                            @foreach($supportedLocales as $locale)
                                <option value="{{ $locale['lang_value'] }}" @if($video->description_lang == $locale['lang_value']) selected @endif>
                                    {{ $locale['lang_name'] }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="seo[h2]" value="{{@$video->seoDetails->h2 ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Keyword:</label>
                            <input type="text" name="seo[keyword]" value="{{@$video->seoDetails->keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Keyword:</label>
                            <input type="text" name="seo[meta_keyword]" value="{{@$video->seoDetails->meta_keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Description:</label>
                            <input type="text" name="seo[desc]" value="{{@$video->seoDetails->desc ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">H2</label>
                            <input type="text" name="seo[de][h2]" value="{{@$video->seoDetails->de->h2 ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Keyword:</label>
                            <input type="text" name="seo[de][keyword]" value="{{@$video->seoDetails->de->keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Keyword:</label>
                            <input type="text" name="seo[de][meta_keyword]" value="{{@$video->seoDetails->de->meta_keyword ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Meta Description:</label>
                            <input type="text" name="seo[de][desc]" value="{{@$video->seoDetails->de->desc ?? ''}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                    ...
                    <div class="col-span-2 grid grid-cols-1 grid-cols-1 gap-6">
                        <div>
                            ...
                            <input id="og-title" type="text" name="seo[og_title]" value="{{@$video->seoDetails->og_title}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="og-desc" class="block text-sm font-medium text-gray-700">OG Description:</label>
                            <input id="og-desc" type="text" name="seo[og_desc]" value="{{@$video->seoDetails->og_desc}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="meta-robot" class="block text-sm font-medium text-gray-700">Meta Robot:</label>
                            <input id="meta-robot" type="text" name="seo[meta_robot]" value="{{@$video->seoDetails->meta_robot}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="cust-url" class="block text-sm font-medium text-gray-700">Custom URL:</label>
                            <input id="cust-url" type="text" name="seo[cust_url]" value="{{@$video->seoDetails->cust_url}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="og-image-url" class="block text-sm font-medium text-gray-700">OG Image URL:</label>
                            <input id="og-image-url" type="text" name="seo[og_image_url]" value="{{@$video->seoDetails->og_image_url}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>

                        <div>
                            <label for="json-id" class="block text-sm font-medium text-gray-700">JSON ID:</label>
                            <input id="json-id" type="text" name="seo[json_id]" value="{{@$video->seoDetails->json_id}}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                        </div>
                    </div>
                </div>
