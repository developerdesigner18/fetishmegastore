...
@section('section_title')
    <strong>{{ __('Promo PreviewVideo') }}</strong>
@endsection
...
@section('section_body')
    ...
    <div class="flex justify-between items-center mb-4">
        <a href="{{ route('add.PreviewVideos') }}">
            <x-button>{{ __('Add Promo Preview Videos') }}</x-button>
        </a>
                ...
                @foreach ($promo_videos as $c)
                    <tr>
                        ...
                        <x-td>
                            ...
                            @endif -->
                            @if($c->getRawOriginal('thumbnail') && file_exists(public_path($c->getRawOriginal('thumbnail'))))
                                <img src="{{ $c->thumbnail }}" alt="thumbnail" class="w-20 h-auto rounded shadow">
                            @else
                                <span class="text-gray-400">N/A</span>
                            @endif
                       ...
                       <x-td>
                            ...
                            <label class="switch">
                                <input 
                                    type="checkbox" 
                                    onchange="confirmStatusChange(this, '{{ route('PreviewVideos.toggleStatus', ['id' => $c->id]) }}')"
                                    {{ $c->status == 1 ? 'checked' : '' }}
                                >
                        ...
                        <x-td>
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('edit.PreviewVideos', ['id' => $c->id]) }}"
                                   class="text-blue-600 hover:text-blue-800">
                                    <i class="fa-solid fa-pencil"></i>
                                </a>
                                <a href="javascript:void(0);" class="text-red-500 hover:text-red-700"
                                   onclick="confirmDelete('{{ route('PreviewVideos.delete', ['id' => $c->id]) }}')">
    ...
    @else
        <p class="text-gray-500">{{ __('No Promo videos in database.') }}</p>
    @endif
...
@push('adminExtraJS')
    ...
    function confirmDelete(url) {
        Swal.fire({
            title: 'Are you sure?',
            text: "This promo videos will be deleted permanently!",
            icon: 'warning',
   ...
   function confirmStatusChange(checkbox, url) {
        ...
        Swal.fire({
            title: 'Change Status?',
            text: "Do you want to turn this videos " + (originalState ? 'ON' : 'OFF') + "?",
            icon: 'question',
            showCancelButton: true,
