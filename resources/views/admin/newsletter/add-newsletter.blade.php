...
@section('section_body')
    ...
    <form method="POST" action="{{route('newsletter.add')}}"
        ...
        <div class="bg-white rounded-lg shadow-sm p-3 text-stone-600">
            ...
            <x-input id="title" class="block mt-0 w-full" type="text" name="title" required/>

             <x-label>{{ __("Description") }}</x-label>
            {{-- <textarea name="description" id="description" class="w-full textarea" rows="20">
            </textarea> --}}
            <x-textarea name="description" class="w-full textarea" rows="20"></x-textarea>
