...
export default function Stories({blog, exploreImage,userrequest,tags,categories}) {
    const settings = {
        dots: true,
        infinite: true,
        speed: 500,
    ...
    return (
        ...
        >
            ...
            <div className="flex w-full -mt-16">
                <form onSubmit={submit}>
                    ...
                    >
                        ...
                        <SecondaryButton
                            className="mt-2"
                            onClick={(e) =>
                                Inertia.visit(route("web.story.index"))
                            }
                        >
                            {__("Reset")}
                        </SecondaryButton>
                    ...
                    {data.map((item, index) => (
                        ...
                        >
                            <div className="flex-shrink-0">
                                <a href={route("web.story.info", { slug: item?.slug })}>
                                    <img
                                        src={item.imageUrl}
                                        alt={item.title}
                                        title={item.title}
                            ...
                            <div className="ml-0 mt-4 md:ml-4 md:mt-0">
                                <h4 className="text-lg font-bold">
                                    <a
                                        ...
                                        href={route("web.story.info", { slug: item?.slug })}
                                    >
                                        {item.title}
                                    </a>
                                </h4>
                ...
                {/*                        alt={item.title}*/}
                {/*                        title={item.title}*/}
                {/*                        className="img-thumbnail wizmage-pattern-bg-img wizmage-cls wizmage-shade-4"*/}
                ...
                {/*            <div className="ml-4">*/}
                {/*                <h4 className="text-lg font-bold">*/}
                {/*                    <a className="font-semibold  dark:text-indigo-100 hover:text-indigo-800 text-indigo-600 dark:hover:text-indigo-400" href={route("web.story.info", {slug: item?.slug})}>{item.title}</a>*/}
                {/*                </h4>*/}
                {/*                <div>*/}
                {/*                    <div className="text-gray-600 mr-2  inline-flex items-center space-x-1 dark:text-zinc-100 text-sm">*/}
                {/*                        <BsTagFill className="w-3"/>*/}
                ...
                {/*                        <span>{item?.tagNames}</span>*/}
                {/*                    </div>*/}
                {/*                    <h1 className="text-justify"><a className="text-gray-500 dark:text-gray-200" href={route("web.story.info", {slug: item.slug})}>{__('More Info...')}</a></h1>*/}
