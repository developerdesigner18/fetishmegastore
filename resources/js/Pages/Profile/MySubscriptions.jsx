...
export default function MySubscribers({ subs }) {
    ...
    const [subscriptions, setSubscriptions] = useState(subs.data); 
    ...
    const handleCancelSubscription = (userId) => {
        if (confirm('Are you sure you want to cancel the subscription?')) {
            ...
            axios
                .post(route('cancel-subscription', { id: userId })) 
                .then((response) => {
                    ...
                    <div className="mt-5 p-4 sm:p-8 bg-white dark:bg-zinc-900 shadow sm:rounded-lg">
                        ...
                        {subs.total === 0 && (
                            <div className="text-xl dark:text-white text-gray-700 flex items-center space-x-4">
                                ...
                                <div>
                                    {__("You haven't subscribed to any channel.")}
                                </div>
                            </div>
                        )}
                            ...
                            {subscriptions?.map((user, index) => (
                                ...
                                >
                                    <div>
                                        <Link
                                            href={route("channel", {
                                                user: user.streamer?.username ?? 'not-found',
                                            })}
                                        >
                                            <img
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-2">
                                            {user.status === 'Active' ? (
                                                <button
                                                    className={`py-1 px-3 text-xs font-semibold bg-red-500 text-white rounded-md hover:bg-red-600 ${processing ? 'opacity-50 cursor-not-allowed' : ''}`}
