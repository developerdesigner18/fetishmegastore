...
export default function Subscribe({ audio }) {
    const { props } = usePage();
    ...
    return (
        <AuthenticatedLayout>
            <Head
                title={__("Unlock Audio: :audioTitle", {
                    audio: audio.title,
                })}
            />
                ...
                <div className="p-4 sm:p-8 bg-gray-100 dark:bg-zinc-900 shadow sm:rounded-lg">
                    ...
                    <div className="border-t pt-5 text-xl text-center font-light mt-8 dark:text-white">
                        <p className="bg-white shadow text-gray-800 font-semibold inline-flex rounded-lg p-2">
                            ...
                            {__("Price: :price tokens", { price: audio.price })}
                        </p>
                        <br />
                            ...
                            {audio.price <= auth.user.tokens ? (
                                <>
                                    ...
                                    >
                                        ...
                                        <div>{__("Purchase Audio : :userBalance ", {
                                            userBalance: audio.title,
                                        })}</div>
                                    </button>
                                    <br />
                                    {processing && (
                                ...
                                <div className="text-gray-700 dark:text-white text-base">
                                    {__(
                                        "You need to charge your token balance to be able to unlock this audio"
                                    )}
                                    <br />
                                    <Link
