...
export default function Upload({videos}) {
    ...
    const deleteVideo = () => {
        Inertia.visit(route("model.delete"), {
            method: "POST",
            data: {video: deleteId},
            preserveState: false,
        });
            ...
            >
                <div className="px-5 py-10 text-center">
                    <h3 className="text-xl mb-3 text-zinc-700 dark:text-white">
                        {__("Are you sure you want to remove this model?")}
                    </h3>
                    ...
                    <div className="mt-5 p-4 sm:p-8 bg-white dark:bg-zinc-900 shadow sm:rounded-lg">
                        <header>
                            ...
                            <p className="mt-1 mb-2 text-sm text-gray-600 dark:text-gray-400">
                                {__("Upload & manage Model for the channel")}
                            </p>

                            <PrimaryButton
                                onClick={(e) =>
                                    Inertia.visit(route("model.upload"))
                                }
                            >
                                {__("Upload Model")}
                            </PrimaryButton>
                        ...
                        {videos.total === 0 && (
                            <div className="text-gray-600 dark:text-white">
                                {__("You didn't upload any Model yet.")}
                            </div>
                        )}
                        ...
                        {videos.total !== 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-x-5 gap-y-10">
                                {videos.data.map((v) => (
                                    ...
                                    >
                                        <img className="w-full rounded-lg aspect-auto" src={v.imageUrl}
                                             alt={v.thumbnail}/>
