...
export default function EditBlogs({ categories, tags, blogs }) {
  ...
  const initCategory = toNumberArray(blogs?.category_id);
  const initTag = toNumberArray(blogs?.tag_id);

  const initSeo =
    typeof blogs?.seo === "string"
      ? JSON.parse(blogs.seo)
      : blogs?.seo ?? {
          h2: "",
          keyword: "",
          meta_keyword: "",
  ...
  const { data, setData, put, processing, errors } = useForm({
    title_en: blogs?.title?.en || "",
    title_de: blogs?.title?.de || "",
    description_en: blogs?.description?.en || "",
    description_de: blogs?.description?.de || "",
    category_id: initCategory,
    tag_id: initTag,
    image: [],
    seo: initSeo,
  });
  ...
  const submit = (e) => {
    e.preventDefault();
    put(route("blogs.update", blogs.id), { forceFormData: true });
  };
      ...
      <div className="lg:flex lg:space-x-10">
        ...
        <div className="p-4 sm:p-8 bg-white w-full dark:bg-zinc-900 shadow sm:rounded-lg">
          <header className="mb-5">
            <h2 className="text-lg font-medium text-gray-600 dark:text-gray-100">
              Edit Blog
            </h2>
          </header>
          ...
          <form onSubmit={submit} encType="multipart/form-data">
            ...
            <Select
              ...
              onChange={(v) => handleSelectChange(v, "category_id")}
              options={categories}
            />
            ...
            <Select
              ...
              value={data.tag_id}
              onChange={(v) => handleSelectChange(v, "tag_id")}
              options={tags}
            />
