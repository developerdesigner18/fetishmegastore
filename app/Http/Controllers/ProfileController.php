...
//        ]);
        return Inertia::render('Profile/Edit', [
            ...
            'status' => session('status'),
            'affiliate_code' => $request->user()->affiliate_code,
    ...
    {  
        ...
        if ($request->hasFile('profilePicture')) {
         ...
         $affiliateCode = $request->affiliate_code;
            ...
            if (!$user->affiliate_user && $affiliateCode) {
                $refUser = User::where('affiliate_code', $affiliateCode)->first();

                if (!$refUser) {
                    return back()->withErrors(['affiliate_code' => 'Affiliate code does not exist.']);
                }

                if ($refUser->id === $user->id) {
                    return back()->withErrors(['affiliate_code' => 'You cannot refer yourself.']);
                }

                $user->affiliate_user = $refUser->id;
                $user->affiliate_code = $refUser->affiliate_code;
                $user->save();
            }
    ...
    {
        $orders = TokenSale::where('user_id', auth()->user()->id)->where('status', 'paid')->paginate(10);
