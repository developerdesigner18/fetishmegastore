...
//    }
    ...
    public function filterVideos(Request $request)
    {
//        $storageDomain = 'https://storage.fetishmegastore.com/';
        $locale = App::getLocale();

        $perPage = 12;

        // --- VIDEO QUERIES ---
        $videosQuery = Video::with('streamer');
        $shortVideosQuery = ShortVideo::with('streamer');

        // --- FILTERS ---
        if ($request->filled('search')) {
            $searchTerm = '%' . $request->search . '%';
            $videosQuery->where('title->en', 'LIKE', $searchTerm);
            $shortVideosQuery->where('title->en', 'LIKE', $searchTerm);
        }
        ...
        $applySort = function ($query) use ($sort) {
            return match ($sort) {
                'Most' => $query->orderByDesc('views'),
                'Recently' => $query->orderByDesc('id'),
                'Older' => $query->orderBy('created_at', 'asc'),
                'Highest' => $query->orderByDesc('price'),
                'Lowest' => $query->orderBy('price'),
                'Only Free' => $query->where('price', 0)->orderByDesc('views'),
                default => $query->orderByDesc('id'),
        ...
        $videosQuery = $applySort($videosQuery);
        $shortVideosQuery = $applySort($shortVideosQuery);

        // --- FETCH DATA ---
        $videos = $videosQuery->get();
        $shortVideos = $shortVideosQuery->get();

        // Merge videos and short videos
        $allVideos = $videos->merge($shortVideos);

        // --- SHUFFLE IF NEEDED ---
        $allVideos = $allVideos->shuffle();
...
//        dd($paginatedVideos->toArray());

        // --- TRANSFORM VIDEOS FOR RESPONSE ---
        $paginatedVideos->getCollection()->transform(function ($video) use($locale) {
            return (object)[
                'id' => $video->id,
                'user_id' => $video->user_id,
                'title' => is_array($video->title) ? ($video->title[$locale] ?? '') : $video->title,
                'slug' => $video->slug,
//                'thumbnail' => $video->thumbnail ? $storageDomain . ltrim($video->thumbnail, '/') : null,
                'thumbnail' => $video->thumbnail ,
                'price' => $video->price,
                'free_for_subs' => $video->free_for_subs ?? null,
                'views' => $video->views,
                'tags' => $video->tags ?? [],
                'category_id' => $video->category_id ?? [],
                'model_id' => $video->model_id ?? [],
                'description' => is_array($video->description) ? ($video->description['en'] ?? '') : $video->description,
                'type' => $video instanceof ShortVideo ? 'short-video' : 'video',
                'videoUrl' => $video->videoUrl ?? null,
                'videoUrl360p' => $video->videoUrl360p ?? null,
                'canBePlayed' => $video->canBePlayed ?? false,
                'canDownload' => $video->canDownload ?? false,
                'isCurrentSubscriber' => $video->isCurrentSubscriber ?? false,
                'isCurrentPurchased' => $video->isCurrentPurchased ?? false,
                'isUserFavorite' => $video->isUserFavorite ?? false,
                'categoryNames' => $video->categoryNames ?? null,
                'selectedCategory' => $video->selectedCategory ?? [],
                'selectedModel' => $video->selectedModel ?? [],
                'modelNames' => $video->modelNames ?? null,
                'videoGIF' => $video->videoGIF ?? null,
                'Str360' => $video->Str360 ?? null,
                'StrGIF' => $video->StrGIF ?? null,
                'videoName' => $video->videoName ?? null,
                'fullVideoLink' => $video->fullVideoLink ?? [],
                'selectedTags' => $video->selectedTags ?? [],
                'seoDetails' => $video->seoDetails ?? null,
                'streamer' => $video->streamer ?? null,
    ...
    public function videosManager(Request $request)
    {
        ...
        $videos = $videos->withSum('sales', 'price')
            ->orderByDesc('id')
