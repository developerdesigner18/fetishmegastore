...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request)
    {
        ...
        return array_merge(parent::share($request), [
            ...
            'pages' => Page::orderByDesc('show_order')->select(['id', 'page_title', 'page_slug'])->get(),
            'recaptcha_key' => opt('RECAPTCHA_KEY'),
            'anyone_live' => User::where('live_status', 'online')->exists(),
            'rtmp_url' => env('RTMP_URL'),
            'current_locale' => Session::has('change_locale') ? Session::get('change_locale') : 'en',
            'streamer_page' => Page::where('page_slug','streamer-guide')->select(['id', 'page_title', 'page_slug'])->first(),
        ]);
    }
}
